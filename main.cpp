...
std::vector<std::complex<double>> dotProduct(const std::vector<std::vector<std::complex<double>>>& R, const std::vector<std::complex<double>>& x) {
    ...
    for (size_t i = 0; i < rows; ++i) {
        ...
        size_t j = 0;
        for (; j + 1 < cols; j += 2) {
            // Store real and imaginary parts in temporary variables
            double R_ij_real_0 = R[i][j].real();
            double R_ij_real_1 = R[i][j + 1].real();
            double R_ij_imag_0 = R[i][j].imag();
            double R_ij_imag_1 = R[i][j + 1].imag();
            double x_j_real_0 = x[j].real();
            double x_j_real_1 = x[j + 1].real();
            double x_j_imag_0 = x[j].imag();
            double x_j_imag_1 = x[j + 1].imag();

            // Temporary arrays to hold real and imaginary parts
            double R_real_arr[2] = { R_ij_real_0, R_ij_real_1 };
            double R_imag_arr[2] = { R_ij_imag_0, R_ij_imag_1 };
            double x_real_arr[2] = { x_j_real_0, x_j_real_1 };
            double x_imag_arr[2] = { x_j_imag_0, x_j_imag_1 };

            // Load real and imaginary parts into NEON registers
            float64x2_t r_real = vld1q_f64(R_real_arr);
            float64x2_t r_imag = vld1q_f64(R_imag_arr);
            float64x2_t x_real = vld1q_f64(x_real_arr);
            float64x2_t x_imag = vld1q_f64(x_imag_arr);

            // Multiply real parts and subtract imaginary parts
            float64x2_t mul_real = vmulq_f64(r_real, x_real);
            float64x2_t mul_imag = vmulq_f64(r_imag, x_imag);
            mul_real = vsubq_f64(mul_real, mul_imag);

            // Multiply and add for imaginary parts
            float64x2_t temp1 = vmulq_f64(r_real, x_imag);
            float64x2_t temp2 = vmulq_f64(r_imag, x_real);
            float64x2_t mul_imag_total = vaddq_f64(temp1, temp2);

            // Horizontal add
            double real_sum[2];
            double imag_sum[2];
            vst1q_f64(real_sum, mul_real);
            vst1q_f64(imag_sum, mul_imag_total);
            total_real += real_sum[0] + real_sum[1];
            total_imag += imag_sum[0] + imag_sum[1];
        }

        // Handle remaining elements
        for (; j < cols; ++j) {
            total_real += R[i][j].real() * x[j].real() - R[i][j].imag() * x[j].imag();
            total_imag += R[i][j].real() * x[j].imag() + R[i][j].imag() * x[j].real();
        }

        z[i] = std::complex<double>(total_real, total_imag);
    }
    return z;
}
*/
